...
dfc_ts = comet.methods.FlexibleLeastSquares(ts_hp, **{'standardizeData': True, 'mu': 50, 'num_cores': 8}).connectivity() # estimate dFC
...
graph_results = Parallel(n_jobs=8)(delayed(comet.graph.compute_graph_measures)(t, features, index, 0.25, False) for t in tqdm(range(features.shape[0])))
