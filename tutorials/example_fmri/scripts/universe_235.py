...
# Estimate dFC
dfc_ts = comet.connectivity.FlexibleLeastSquares(time_series=ts_hp, standardizeData=True, mu=50, num_cores=8, progress_bar=False).estimate()
...
def compute_graph_measures(t, features, index, density, binarise):
    ...
    graph_results = comet.graph.participation_coef(G, ci='louvain')

    return graph_results

graph_results = Parallel(n_jobs=4)(delayed(compute_graph_measures)(t, features, index, 0.25, False) for t in range(features.shape[0]))
