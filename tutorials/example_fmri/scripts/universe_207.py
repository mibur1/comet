...
# Estimate dFC
dfc_ts = comet.connectivity.FlexibleLeastSquares(ts_hp, **{'standardizeData': True, 'mu': 50, 'num_cores': 8}).estimate()
...
def compute_graph_measures(t, features, index, density, binarise):
    ...
    graph_results = comet.graph.clustering_coef(W, **{})

    return graph_results

graph_results = Parallel(n_jobs=4)(delayed(compute_graph_measures)(t, features, index, 0.25, True) for t in tqdm(range(features.shape[0])))
